...
class RunSimulation:
    ...
    def __init__(self, dir_name, wavelength, cyt_absorb):
        ...
        source = [
            mp.Source(
                mp.ContinuousSource(frequency=self.frequency, fwidth=self.pulse_width),
                compone=mp.Ez,
...
ChloroA_singleCyto = RunSimulation("Point Source", 300, 0)
#ChloroB_singleCyto = RunSimulation("ChloroB - 640nm Single Layer Cytoplasm", 0.640, 0)

#UVB_singleThyla = RunSimulation("UV-A 350nm SingleLayer Cytoplasm", 0.350, 0)
#ChloroA_singleThyla = RunSimulation("ChloroA - 430nm Single Layer Cytoplasm", 0.430, 0)
#ChloroB_SingleThyla = RunSimulation("ChloroB - 640nm Single Layer Cytoplasm", 0.640, 0)
